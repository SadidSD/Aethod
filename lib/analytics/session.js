...
const SESSION_KEY = "aeethod_v2_sid";
const SESSION_START_KEY = "aeethod_v2_sst";
const SESSION_LAST_ACTIVE_KEY = "aeethod_v2_s_last_active";
const SESSION_ATTR_KEY = "aeethod_v2_s_attr";
const LEGACY_SESSION_KEYS = ["aeethod_sid", "aeethod_sst", "aeethod_s_last_active", "aeethod_s_attr"];
 ...
 * Automatically expires sessions after 30 minutes of complete inactivity.
 * Purges pre-reset legacy session keys to avoid baseline contamination.
...
export function getOrCreateSessionId() {
  ...
  try {
    // Purge legacy pre-reset session data if present
    ...
    const isStale = lastActiveStr && (now - Number(lastActiveStr)) > SESSION_TIMEOUT_MS;

    if (existing && existing.length >= 10 && existing.length <= 64 && !isStale) {
...
/**
 * Records activity on the active session.
 */
