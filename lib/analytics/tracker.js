...
class AnalyticsTracker {
  ...
  trackPageView(path, search = "") {
    ...
    const currentUtm = extractUtmParams(search);
    const aiDetection = detectAiReferral(rawReferrer, search);
    const currentTrafficSource = classifyTrafficSource(rawReferrer, search, window.location.origin);

    // Retrieve or initialize preserved session attribution
    let sessionAttr = isNewSession ? null : getSessionAttribution();

    const isCurrentSourceReliable = currentTrafficSource && currentTrafficSource !== "Direct";
    ...
    if (!sessionAttr || (!isStoredSourceReliable && isCurrentSourceReliable)) {
      ...
      sessionAttr = {
        ...
        aiPlatform: aiDetection.isAiReferral ? aiDetection.platform : null,
        aiAttributionType: aiDetection.attributionType,
    ...
    const payload = {
      ...
      aiAttributionType: sessionAttr.aiAttributionType || null,
