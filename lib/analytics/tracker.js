...
class AnalyticsTracker {
  ...
  trackPageView(path, search = "") {
    ...
    // Detect partner referral from active search or referrer
    let partnerId = sessionAttr?.referralPartner || null;
    const lowerSearch = String(activeSearch).toLowerCase();
    const lowerRef = String(rawReferrer).toLowerCase();
    if (!partnerId) {
      if (
        lowerSearch.includes("ref=rng-gamez") ||
        lowerSearch.includes("rng-gamez") ||
        lowerRef.includes("rng-gamez") ||
        lowerRef.includes("rnggamez")
      ) {
        partnerId = "rng-gamez";
      } else if (
        lowerSearch.includes("murakkaz") ||
        lowerRef.includes("murakkaz") ||
        currentUtm?.utm_campaign === "footer_credit"
      ) {
        partnerId = "murakkaz";
      }
    }
    ...
    const isCurrentSourceReliable = (resolved.source && resolved.source !== "Direct" && resolved.source !== "Bot") || Boolean(partnerId);
    const isStoredSourceReliable = (sessionAttr?.trafficSource && sessionAttr.trafficSource !== "Direct") || Boolean(sessionAttr?.referralPartner);
    ...
    if (!sessionAttr || (!isStoredSourceReliable && isCurrentSourceReliable)) {
      ...
      sessionAttr = {
        ...
        referralPartner: partnerId || sessionAttr?.referralPartner || null,
        ...
        detectionMethod: resolved.detectionMethod || (resolved.source === "AI Referral" ? "referrer" : partnerId ? "partner_ref" : null),
        ...
        landingPage: sessionAttr?.landingPage || (activeSearch ? `${path}${activeSearch.startsWith("?") ? activeSearch : `?${activeSearch}`}` : path),
      ...
      try {
        if (typeof window !== "undefined" && isCurrentSourceReliable) {
          const storedVisitorAttr = localStorage.getItem("aeethod_v2_first_touch");
          if (!storedVisitorAttr) {
            localStorage.setItem("aeethod_v2_first_touch", JSON.stringify(sessionAttr));
          }
        }
      } catch {
        // Storage restricted, ignore
      }
    } else if (partnerId && !sessionAttr.referralPartner) {
      sessionAttr.referralPartner = partnerId;
      setSessionAttribution(sessionAttr);
    }
    ...
    const payload = {
      ...
      referralPartner: sessionAttr?.referralPartner || partnerId || null,
