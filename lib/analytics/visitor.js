...
const VISITOR_KEY = "aeethod_v2_vid";
const LEGACY_VISITOR_KEY = "aeethod_vid";
 ...
 * Purges pre-reset legacy visitor keys to avoid baseline contamination.
...
export function getOrCreateVisitorId() {
  ...
  try {
    // Purge legacy pre-reset visitor identity if present
    if (localStorage.getItem(LEGACY_VISITOR_KEY)) {
      localStorage.removeItem(LEGACY_VISITOR_KEY);
      localStorage.removeItem("aeethod_v_first_touch");
    }
