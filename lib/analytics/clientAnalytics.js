...
/**
 * Fetch dedicated AI Referral analytics.
 *
 * @param {Object} options
 * @param {"today"|"yesterday"|"7d"|"30d"|"custom"} [options.range="7d"]
 ...
 * @param {AbortSignal} [options.signal]
 * @returns {Promise<import("./types").AiReferralPayload>}
 */
export async function fetchAiReferrals({ range = "7d", from, to, signal } = {}) {
  let url = `/api/admin/analytics/ai-referrals?range=${encodeURIComponent(range)}`;
  if (range === "custom" && from && to) {
    url += `&from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}`;
  }
  const res = await fetch(url, {
  ...
  if (res.status === 401) {
    if (typeof window !== "undefined") window.location.href = "/yamal19";
    throw new Error("Session expired. Please log in again.");
  }
  if (!res.ok) throw new Error("Failed to fetch AI referral data");
  const json = await res.json();
  return json.data;
}
