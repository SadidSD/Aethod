...
import { fetchAdminAnalytics } from "./clientAnalytics";

/**
 * Clean data layer fetcher pointing directly to the real Admin Analytics API.
 * Preserved for backwards compatibility with any existing calls.
 *
 * @param {"today"|"yesterday"|"7d"|"30d"|"custom"} range
 * @param {string} [from]
 * @param {string} [to]
 * @returns {Promise<any>}
 */
export async function fetchAnalyticsData(range = "7d", from, to) {
  return fetchAdminAnalytics({ range, from, to });
}
