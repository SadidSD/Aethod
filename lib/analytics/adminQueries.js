...
export const PARTNER_CONFIGS = [
  {
    ...
    trackingUrl: "https://aeethod.com/?ref=rng-gamez",
  ...
  },
  {
    ...
    trackingUrl: "https://aeethod.com/?utm_source=murakkaz.com&utm_medium=referral&utm_campaign=footer_credit&utm_content=crafted_by_aeethod",
...
/**
 * Computes dedicated analytics metrics for the 2 partner referral links.
 * Isolates visits, sessions, pageviews, dwell time, and conversion actions.
 */
export function computePartnerTrackers(sessions, pageViews, events) {
  ...
  return PARTNER_CONFIGS.map((config) => {
    const matchingSessions = legitimate.filter(config.matches);
    const sessionIds = new Set(matchingSessions.map((s) => s.session_id));
    const visitorIds = new Set(matchingSessions.map((s) => s.visitor_id).filter(Boolean));
    ...
    const conversionEvents = matchingEvents.filter((e) =>
      ["inquiry_submitted", "call_booked", "contact_form_submit"].includes(e.event_name)
    );
