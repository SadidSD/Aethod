...
import { getAiPlatformFromRecord, detectAiReferral, AI_PLATFORMS } from "./aiPlatforms.js";
...
export async function fetchRawAnalyticsData(supabase, from, to) {
  ...
  const [sessionsRes, pageViewsRes, eventsRes] = await Promise.all([
    supabase
      .from("sessions")
      .select("session_id, visitor_id, started_at, last_activity_at, device_type, traffic_source, referrer, browser, operating_system, country, city, utm_source, utm_medium, utm_campaign")
...
function computePeriodMetrics(sessions, pageViews, events) {
  ...
  // Use difference between last_activity_at and started_at per session, supplemented by pageview duration sum
  let totalDurationSeconds = 0;
  for (const s of sessions) {
    const started = new Date(s.started_at).getTime();
    const lastActive = new Date(s.last_activity_at).getTime();
    const sessionSpan = Math.max(0, (lastActive - started) / 1000);
  ...
  const bounceRateNum = totalSessions > 0 ? (singlePageSessions / totalSessions) * 100 : 0;
...
export function computeTrafficSources(sessions) {
  ...
  for (const s of sessions) {
    ...
    if (rawSource !== "AI Referral" && s.referrer) {
      ...
      if (detectAiReferral(s.referrer, dummySearch).isAiReferral) {
...
export function computeGeography(sessions) {
  ...
  const countries = Object.entries(countryMap)
    .map(([country, count]) => ({
      country,
      code: country.length === 2 ? country.toUpperCase() : country.slice(0, 2).toUpperCase(),
      sessions: count,
      percentage: total > 0 ? Number(((count / total) * 100).toFixed(1)) : 0,
    }))
  ...
  const cities = Object.entries(cityMap)
    .map(([cityKey, count]) => {
      ...
      return {
        city,
        country: country.length === 2 ? country.toUpperCase() : country.slice(0, 2).toUpperCase(),
        sessions: count,
      };
    })
    .sort((a, b) => b.sessions - a.sessions)
...
export function computeTechnology(sessions) {
  const browserMap = { Chrome: 0, Safari: 0, Edge: 0, Firefox: 0, Other: 0 };
  const osMap = { macOS: 0, Windows: 0, iOS: 0, Android: 0, Linux: 0, Other: 0 };
  const total = sessions.length;
...
export function computeAiReferralAnalytics(sessions, pageViews, events) {
  ...
  for (const s of aiSessions) {
    const platformName = getAiPlatformFromRecord(s.referrer, s.utm_source, s.utm_medium);
    if (!platformMap[platformName]) {
      const matchedConfig = AI_PLATFORMS.find((p) => p.name === platformName);
