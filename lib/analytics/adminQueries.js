...
export async function fetchRawAnalyticsData(supabase, from, to) {
  ...
  const [sessionsRes, pageViewsRes, eventsRes] = await Promise.all([
    supabase
      .from("sessions")
      .select("session_id, visitor_id, started_at, last_activity_at, device_type, traffic_source, browser, operating_system, country, city, utm_source, utm_medium, utm_campaign")
...
function computePeriodMetrics(sessions, pageViews, events) {
  ...
  const conversionEventNames = new Set(["inquiry_submitted", "call_booked", "contact_form_submit"]);
...
export function computeFunnel(sessions, pageViews, events) {
  ...
  for (const e of events) {
    ...
    if (name === "explore_services" || name === "view_work") {
      stage2Sessions.add(e.session_id);
    }
    if (name === "start_contact") {
      stage3Sessions.add(e.session_id);
    }
    if (name === "inquiry_submitted" || name === "call_booked" || name === "contact_form_submit") {
...
export function computeTrafficSources(sessions) {
  ...
  for (const s of sessions) {
    const rawSource = s.traffic_source || "Direct";
