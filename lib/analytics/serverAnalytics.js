...
export function resolveAcquisition({
  ...
  // 6. Partner Referral & Campaign Tracking (e.g. ?ref=rng-gamez or ?utm_source=murakkaz.com)
  const refParam = params.get("ref") || params.get("reference");
  const utmSource = params.get("utm_source");
  const utmMedium = params.get("utm_medium");
  const utmCampaign = params.get("utm_campaign");

  if (refParam || utmSource || utmMedium === "referral") {
    const isRngGamez = refParam === "rng-gamez" || utmSource === "rng-gamez" || utmCampaign === "rng-gamez";
    const isMurakkaz = utmSource?.includes("murakkaz") || utmCampaign === "footer_credit";

    let partnerHost = utmSource || (refParam ? `${refParam}.partner` : "partner-referral");
    if (isRngGamez) partnerHost = "rnggamez.com";
    if (isMurakkaz) partnerHost = "murakkaz.com";
    ...
    return {
      ...
      aiPlatform: null,
      confidence: "high",
      detectionMethod: refParam ? "ref_param" : "utm",
      referrerHost: partnerHost,
      referrerPath: null,
      unknownReferrerHost: null,
      reason: isRngGamez
        ? "partner_referral:rng-gamez"
        : isMurakkaz
        ? "partner_referral:murakkaz"
        : `referral_campaign:${utmSource || refParam}`,
      conflict: null,
    };
  }

  // 7. Direct / Unattributed (No referrer, no campaign)
  return {
    source: "Direct",
    isCrawler: false,
    aiPlatform: null,
