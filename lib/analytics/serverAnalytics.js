...
export function resolveClientIp(request) {
  ...
  // 1. Cloudflare CDN (Authoritative behind Cloudflare)
  ...
  // 5. Standard proxy forwarded header (client IP is leftmost)
  const forwarded = request.headers.get("x-forwarded-for");
  if (forwarded) {
    const first = cleanCandidateIp(forwarded.split(",")[0]);
    if (first) return first;
  }
...
// In-memory cache for IP geo lookups
const IP_GEO_CACHE = new Map();
const IP_GEO_CACHE_TTL_MS = 60 * 60 * 1000; // 1 hour

/**
 * Canonical Geolocation Resolver: resolveGeoFromIp(ip)
 * Returns ONE coherent geographic object from a single source.
 * NEVER fabricates city or country.
 *
 * @param {string} ip
 * @returns {Promise<{
...
export async function resolveGeoFromIp(ip) {
  ...
  if (cached && Date.now() - cached.timestamp < IP_GEO_CACHE_TTL_MS) {
    return cached.data;
  }

  try {
    const response = await fetch(
      `http://ip-api.com/json/${encodeURIComponent(ip)}?fields=status,message,country,countryCode,region,regionName,city,timezone`,
    ...
    if (data.status === "fail") return nullGeo;

    const resolvedCountry = resolveCountry(data.countryCode || data.country);
    const rawCity = normalizeCity(data.city);

    // Validate consistency
    const isConsistent = validateGeoConsistency(resolvedCountry.code, rawCity);
    const validCity = isConsistent ? rawCity : null;

    const result = {
      countryCode: resolvedCountry.code,
      countryName: resolvedCountry.name !== "Unknown" ? resolvedCountry.name : null,
      regionCode: data.region || null,
      regionName: data.regionName || null,
      city: validCity,
      metro: validCity, // Metro defaults to city when dedicated metro unavailable
      timezone: data.timezone || null,
    };

    IP_GEO_CACHE.set(ip, { data: result, timestamp: Date.now() });
    return result;
  } catch {
    return nullGeo;
  }
...
/**
 * Extracts non-sensitive geographic location from Edge headers or resolves from IP.
 * Hierarchy:
 * 1. Trusted Edge Headers (Vercel / Cloudflare)
 * 2. IP Geolocation Resolver (fallback for VPS/local)
 * 3. Safe null fallback
...
export async function extractGeoFromHeaders(request) {
  ...
  if (cityHeader) {
    try {
      cityHeader = decodeURIComponent(cityHeader);
    } catch {
      // ignore decode error
    }
  }
  ...
  if (countryHeader && (countryHeader.toUpperCase() === "UN" || countryHeader.toUpperCase() === "XX")) {
    countryHeader = null;
  }

  const resolved = resolveCountry(countryHeader);
  let validCity = normalizeCity(cityHeader);

  // Validate consistency of Edge header data
  if (resolved.code && validCity && !validateGeoConsistency(resolved.code, validCity)) {
    // Drop inconsistent city to preserve integrity
    validCity = null;
  }

  // If Edge headers provided country, return it
  if (resolved.code || (resolved.name && resolved.name !== "Unknown")) {
    const finalCountryName = resolved.name !== "Unknown" ? resolved.name : null;
    return {
      countryCode: resolved.code,
      countryName: finalCountryName,
      regionCode: regionHeader || null,
      regionName: null,
      city: validCity,
      metro: validCity,
      timezone: timezoneHeader || null,
      country: finalCountryName,
    };
  }

  // If Edge headers absent, attempt resolution from client IP
  const clientIp = resolveClientIp(request);
  if (!isPrivateIp(clientIp)) {
    const geoFromIp = await resolveGeoFromIp(clientIp);
    return {
      countryCode: geoFromIp.countryCode,
      countryName: geoFromIp.countryName,
      regionCode: geoFromIp.regionCode,
      regionName: geoFromIp.regionName,
      city: geoFromIp.city,
      metro: geoFromIp.metro,
      timezone: geoFromIp.timezone,
      country: geoFromIp.countryName,
    };
  }
