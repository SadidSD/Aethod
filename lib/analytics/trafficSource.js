...
export function classifyTrafficSource(referrer, search, currentOrigin = "") {
  ...
  const hasPaidAdId = params?.has("gclid") || params?.has("fbclid") || params?.has("ttclid") || params?.has("wbraid");
  ...
  // 2. AI Referral Check (reliable known AI domains or explicit AI UTM tags)
  const aiDetection = detectAiReferral(referrer, params);
  if (aiDetection.isAiReferral) {
    return "AI Referral";
  }

  // 3. Direct Check (No referrer, or internal self-referral)
  ...
  try {
    ...
    if (currentOrigin && refUrl.origin === currentOrigin) {
      return "Direct";
    }

    const host = refUrl.hostname.toLowerCase();

    // 4. Organic Search Check
