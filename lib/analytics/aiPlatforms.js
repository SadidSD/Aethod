...
export const KNOWN_AI_UTM_MEDIUMS = new Set([
  "ai_referral",
  "ai",
  "chat",
  "chatbot",
...
export function detectAIPlatform(referrer, search = "") {
  ...
  try {
    ...
    rawUtmMedium = decodeURIComponent(params.get("utm_medium") || "").trim().toLowerCase();
  } catch {
    rawUtmSource = (params.get("utm_source") || "").trim().toLowerCase();
    rawUtmMedium = (params.get("utm_medium") || "").trim().toLowerCase();
  }

  // Normalize UTM source: strip protocol if pasted, trailing slashes, www. prefix
  const utmSource = rawUtmSource
    .replace(/^https?:\/\//, "")
    .replace(/\/+$/, "")
    .replace(/^www\./, "");
  const utmMedium = rawUtmMedium.replace(/\/+$/, "");
    ...
    for (const p of AI_PLATFORMS) {
      ...
      if (norm.isApp && p.appPackages && p.appPackages.includes(norm.appPackage)) {
        return {
          ...
          detectionMethod: "other_verified_signal",
      ...
      // B. Path Rules match (e.g. claude.ai/referral, bing.com/chat, bing.com/copilot, chatgpt.com/c/)
      if (p.pathRules) {
        for (const rule of p.pathRules) {
          ...
          if (matchesHost) {
            const prefixes = Array.isArray(rule.pathPrefixes)
              ...
              ? [rule.pathPrefix]
              : [];
            const matchesPath = prefixes.some((prefix) => {
              const cleanPrefix = prefix.endsWith("/") ? prefix.slice(0, -1) : prefix;
              return (
                norm.pathname === cleanPrefix ||
                norm.pathname.startsWith(cleanPrefix + "/") ||
                norm.pathname.startsWith(cleanPrefix + "?")
              );
            });
            if (matchesPath) {
  ...
  // 3. Evaluate AI utm_medium campaign signal
  if (utmMedium && KNOWN_AI_UTM_MEDIUMS.has(utmMedium)) {
    // Try to match platform by utmSource or return Other AI
    let matchedPlatform = null;
    if (utmSource) {
      for (const p of AI_PLATFORMS) {
        const allNames = [p.name.toLowerCase(), p.id.toLowerCase(), ...(p.aliases || []).map(a => a.toLowerCase())];
        if (allNames.some(name => utmSource.includes(name))) {
          matchedPlatform = p;
          break;
        }
      }
    ...
    return {
      isAiReferral: true,
      attributionType: AI_ATTRIBUTION_TYPES.VERIFIED_AI_REFERRAL,
      platform: matchedPlatform ? (matchedPlatform.canonicalName || matchedPlatform.name) : "Other AI",
      platformId: matchedPlatform ? matchedPlatform.id : "other_ai",
      sourceType: "utm_parameter",
      detectionMethod: "utm",
      domain: utmSource || "utm_medium:" + utmMedium,
      color: matchedPlatform?.color || "#10B981",
      path: null,
    };
  }

  // 4. No verified AI referral signal present
  return {
    isAiReferral: false,
