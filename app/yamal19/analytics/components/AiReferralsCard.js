...
export default function AiReferralsCard({ aiReferrals }) {
  const safeData = aiReferrals || {
    ...
    platforms: [],
    funnel: [
      { stage: "Website Visits", count: 0, percentage: 0, dropoff: null },
      { stage: "Explored Services / Work", count: 0, percentage: 0, dropoff: 0 },
      { stage: "Started Contact", count: 0, percentage: 0, dropoff: 0 },
      { stage: "Inquiry / Call Booked", count: 0, percentage: 0, dropoff: 0 },
    ],
  };
        ...
        <div className={styles.headerLeft}>
          ...
          <div>
            ...
            <p className={styles.cardSubtext}>
              Telemetry from conversational AI search engines and LLM agents (ChatGPT, Perplexity, Claude, Gemini, Copilot)
            </p>
          </div>
        </div>
        <div className={styles.headerBadge}>
          <span className={styles.liveSignalDot} />
          <span>{hasTraffic ? `${aiSessions} AI Sessions Attributed` : "Verified AI Signal Layer"}</span>
        </div>
      </div>
      ...
      <div className={styles.tableSection}>
        ...
        {platforms.length > 0 ? (
          ...
          <div className={styles.emptyState}>
            <p className={styles.emptyStateTitle}>Zero AI Referrals In Period</p>
            <p className={styles.emptyStateDesc}>
              No conversational AI search engine referrers (ChatGPT, Perplexity, Gemini, Claude, Copilot) or AI campaigns were detected within this date range. Signals are strictly verified with zero heuristic guessing.
            </p>
          </div>
        )}
      </div>
